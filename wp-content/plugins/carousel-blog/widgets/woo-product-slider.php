	...
	protected function _register_controls() {
		...
		/*$this->add_control(
			...
			[
				...
				'label_block' => false,
				'button_type' => 'danger',
				'text' => __( '<a style="color: #fff; font-size: 12px; padding: 0 10px; height: 100%; display: block; line-height: 28px;" href="'.BDD_PRO_LINK.'" target="_blank" >Buy Pro</a>', 'plugin-domain' ),
			]
		);
		...
		$product_lists = bdd_get_product_lists();
		$this->add_control(
        ...
        if( $products ){
			...
			foreach( $products as $product ){
				...
				if( $template_style === 'default' ){
					require( BDD_PATH . 'templates/style-1/template.php' );
				}
			}
		}
		echo "</div>";
