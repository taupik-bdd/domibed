...
if( ! class_exists( 'BDD__Usage_Feedback') ) {
	
	class BDD__Usage_Feedback {
		
		private $BDD__VERSION = '0.9.0';
		private $home_url = '';
		private $plugin_file = '';
		private $plugin_name = '';
		...
		public function init() {
			global $pagenow;
				...
				add_action( 'wp_ajax_bdd_goodbye_form', array( $this, 'goodbye_form_callback' ) );
		...
		public function admin_enqueue_scripts(){
			wp_enqueue_style('wbps-admin-style', BDD__URL . 'admin/assets/css/admin.css', array(), BDD__VERSION, 'all');
		...
		public function deactivate_this_plugin() {
			...
			if( false !== get_option( 'bdd_deactivation_reason_' . $this->plugin_name ) ) {
				$body['deactivation_reason'] = get_option( 'bdd_deactivation_reason_' . $this->plugin_name );
				delete_option('bdd_deactivation_reason_' . $this->plugin_name);
			}
			if( false !== get_option( 'bdd_deactivation_details_' . $this->plugin_name ) ) {
				$body['deactivation_details'] = get_option( 'bdd_deactivation_details_' . $this->plugin_name );
				delete_option('bdd_deactivation_details_' . $this->plugin_name);
			}

			if( false !== get_option( 'bdd_deactivation_email_' . $this->plugin_name ) ) {
				$body['deactivation_email'] = get_option( 'bdd_deactivation_email_' . $this->plugin_name );
				delete_option('bdd_deactivation_email_' . $this->plugin_name);
			}

			if( false !== get_option( 'bdd_deactivation_main_reason_' . $this->plugin_name ) ) {
				$body['deactivation_main_reason'] = get_option( 'bdd_deactivation_main_reason_' . $this->plugin_name );
				delete_option('bdd_deactivation_main_reason_' . $this->plugin_name);
			}
		...
		public function form_filterable_text() {
			...
			return apply_filters( 'bdd_form_text_' . esc_attr( $this->plugin_name ), $form );
		}
		...
		public function goodbye_ajax() {
			...
			if( is_array( $form['options'] ) ) {
				...
				$html .= '</p><div id="bdd_additional_content" style="display:none;"><label for="wpce-goodbye-reasons">' . esc_html( $form['email'] ) .'</label><br><input type="email" name="wpce-goodbye-email" id="wpce-goodbye-email" value="'.get_option('admin_email').'" /> (Optional)';
				...
				jQuery(document).ready(function($){
					 $(document).on('change', '#wpb-goodbye-form-woo-product-slider-for-elementor input[type=radio]', function() { 
						if($(this).val()=='Deactivating Temporarily' || $(this).val()=='Upgrading to Pro'){
							$('#bdd_additional_content').hide();
						}else{
							$('#bdd_additional_content').show();
						}
					...
					$('#wpb-goodbye-link-wpce-<?php echo esc_attr( $this->plugin_name ); ?>').on('click',function(){
						...
						$('#wpb-submit-form').on('click', function(e){
							...
							var data = {
								'action': 'bdd_goodbye_form',
								'values': values,
								'main_reason': deactivate_main_reason,
								'details': details,
								'email': email,
								'security': '<?php echo wp_create_nonce ( 'bdd_goodbye_form' ); ?>',
								'dataType': 'json'
							}
		...
		public function goodbye_form_callback() {
			//check_ajax_referer( 'bdd_goodbye_form', 'security' );
			if( isset( $_POST['values'] ) ) {
				$values = json_encode( wp_unslash( $_POST['values'] ) );
				update_option( 'bdd_deactivation_reason_' . $this->plugin_name, $values );
			}
			if( isset( $_POST['details'] ) ) {
				$details = sanitize_text_field( $_POST['details'] );
				update_option( 'bdd_deactivation_details_' . $this->plugin_name, $details );
			}

			if( isset( $_POST['email'] ) ) {
				$email = sanitize_email( $_POST['email'] );
				update_option( 'bdd_deactivation_email_' . $this->plugin_name, $email );
			}

			if( isset( $_POST['main_reason'] ) ) {
				$main_reason = sanitize_text_field( $_POST['main_reason'] );
				update_option( 'bdd_deactivation_main_reason_' . $this->plugin_name, $main_reason );
			}
