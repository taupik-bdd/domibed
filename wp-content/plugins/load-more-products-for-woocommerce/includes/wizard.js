...
var BeRocket_wizard_check = function (
    category_page_url,
    input_data,
    execute_function
) {
    ...
    function br_remove_products_classes(array) {
        ...
        array.forEach(function (item, i, arr) {
            if (
                item != "type-product" &&
                item != "status-publish" &&
                item != "product_cat-berocketselectors" &&
                item != "instock" &&
                item != "taxable" &&
                item != "shipping-taxable" &&
                item != "purchasable" &&
    ...
    var products = [
        "BeRocketSelectorsTest10",
        "BeRocketSelectorsTest11",
        "BeRocketSelectorsTest12",
        "BeRocketSelectorsTest13",
        "BeRocketSelectorsTest14",
        "BeRocketSelectorsTest15",
        "BeRocketSelectorsTest16",
        "BeRocketSelectorsTest17",
        "BeRocketSelectorsTest18",
        "BeRocketSelectorsTest19",
        "BeRocketSelectorsTest20",
        "BeRocketSelectorsTest21",
        "BeRocketSelectorsTest22",
        "BeRocketSelectorsTest23",
        "BeRocketSelectorsTest24",
        "BeRocketSelectorsTest25",
        "BeRocketSelectorsTest26",
        "BeRocketSelectorsTest27",
    ];
    ...
    i = 0;
    while (
        $elem.next().find(search_next_product).length == 0 &&
    ...
    BeRocket_wizard_selectors.product =
        $elem.prop("tagName").toLowerCase() + "." + result_classes.join(".");
    ...
    do {
        ...
        checkClass =
            $products2.prop("tagName").toLowerCase() +
            "." +
            checkClass.join(".");
    ...
    if (is_pagination) {
        ...
        if (checkClass.length > 0) {
            checkClass =
                $pagination.prop("tagName").toLowerCase() +
                "." +
                checkClass.join(".");
    ...
    if ($input_data.find(".woocommerce-result-count").length) {
        ...
        var products_count = [
            ...
            ':contains("Showing 1–3 of 18 results")',
            ':contains("3 results"), :contains("3results")',
            ':contains("3 products"), :contains("3products")',
            ':contains("18 Product"), :contains("18Product")',
        ];
        ...
        if (is_products_count) {
            ...
            if ($products_count.length > 0) {
                ...
                if (typeof checkClass == "string") {
                    ...
                    checkClass = br_remove_empty_classes(checkClass);
                    checkClass =
                        $products_count.prop("tagName").toLowerCase() +
                        "." +
                        checkClass.join(".");
    ...
    if (is_pagination) {
        ...
        if ($next.length == 0) {
            ...
            if ($second_page.length > 0) {
                ...
                $next = $pagination.find(
                    '[href*="' + second_href + '"]:not(:contains("2"))'
                );
            }
        }
        if ($next.length == 0) {
        ...
        jQuery
            .get(current_url, function (data) {
                ...
                var $pagination6 = $data.find(
                    BeRocket_wizard_selectors.pagination
                );
                if ($pagination6.length > 0) {
                    ...
                    if ($prev.length == 0) {
                        ...
                        do {
                            ...
                            !$second_page.is("a") &&
                            $second_page.length > 0
                        );
                        if ($second_page.length > 0) {
                            var second_href = $second_page.attr("href");
                            $prev = $pagination6.find(
                                '[href*="' +
                                    second_href +
                                    '"]:not(:contains("5"))'
                            );
...
jQuery(document).on("click", ".berocket_autoselector", function (event) {
    ...
    BeRocket_current_autoselector_block = jQuery(this).parents(
        ".berocket_wizard_autoselectors"
    );
    BeRocket_init_autoselectors();
    jQuery
        .get(
            berocket_wizard_autoselect.ajaxurl,
            { action: "berocket_wizard_selector_start" },
            function (url) {
                if (BeRocket_autoselector_stop) {
                    berocket_wizard_autoselector(true);
                    return;
                }
                BeRocket_set_autoselector_load_position(
                    45,
                    berocket_wizard_autoselect.getting_selectors,
                    60
                );
                jQuery
                    .get(url, function (data) {
                        ...
                        BeRocket_wizard_check(
                            url,
                            data,
                            berocket_wizard_autoselector
                        );
                    })
                    .error(function () {
                        berocket_wizard_autoselector_end(
...
jQuery(document).on("click", ".berocket_autoselector_seo", function (event) {
    event.preventDefault();
    jQuery(this)
        .removeClass("berocket_autoselector_seo")
        .addClass("berocket_autoselector");
    var seoplugins = jQuery(this).data("seoplugins");
    if (
        confirm(
            "It seems that you have some SEO plugin active. Please disable it before continue.\n\nPlugin(s): " +
                seoplugins +
...
function BeRocket_set_autoselector_load_position(
    position,
    position_name,
    next_position
) {
    if (jQuery(".berocket_wizard_autoselectors ol li .fa-spin").length) {
        jQuery(".berocket_wizard_autoselectors ol li .fa-spin")
    ...
    if (position_name) {
        jQuery(
            ".berocket_wizard_autoselectors ol li span:contains(" +
                position_name +
                ")"
        )
            .parent()
    ...
    if (
        ...
        functions.percent != "" &&
        typeof window[functions.percent] == "function"
    ) {
        window[functions.percent](position, position_name, next_position);
    }
    var $block = jQuery(BeRocket_current_autoselector_block);
    $block
        .find(".berocket_autoselector_load .berocket_line")
        .finish()
        .css("width", position + "%");
    if (typeof position_name != "undefined") {
        $block
            .find(".berocket_autoselector_load .berocket_autoselector_action")
            .text(position_name);
    }
    if (typeof next_position != "undefined") {
        $block
            .find(".berocket_autoselector_load .berocket_line")
            .animate({ width: next_position + "%" }, 30000);
    }
}
function BeRocket_init_autoselectors() {
    var $block = jQuery(BeRocket_current_autoselector_block);
    jQuery(".berocket_wizard_autoselectors .berocket_autoselector").prop(
        "disabled",
        true
    );
    $block
    ...
    BeRocket_set_autoselector_load_position(
        0,
        berocket_wizard_autoselect.creating_products,
        45
    );
    $block
        .find(".berocket_autoselector_load .berocket_line")
...
function berocket_wizard_autoselector(status) {
    if (status) {
        ...
        berocket_wizard_set_val_to_input(
            inputs.products,
            BeRocket_wizard_selectors.products
        );
        berocket_wizard_set_val_to_input(
            inputs.product,
            BeRocket_wizard_selectors.product
        ...
        berocket_wizard_set_val_to_input(
            inputs.next_page,
            BeRocket_wizard_selectors.pagination_next
        );
        berocket_wizard_set_val_to_input(
            inputs.prev_page,
            BeRocket_wizard_selectors.pagination_prev
        );
        berocket_wizard_set_val_to_input(
            inputs.result_count,
            BeRocket_wizard_selectors.products_count
        );
        BeRocket_set_autoselector_load_position(
            75,
            berocket_wizard_autoselect.removing_products,
            100
        );
        berocket_wizard_autoselector_end(
            berocket_wizard_autoselector_end_success
        );
    } else {
...
function berocket_wizard_autoselector_end(function_after_end) {
    jQuery
        .get(
            berocket_wizard_autoselect.ajaxurl,
            { action: "berocket_wizard_selector_end" },
            function () {
                function_after_end();
            }
        )
...
function berocket_wizard_autoselector_end_success() {
    ...
    if (
        functions.success != "" &&
        typeof window[functions.success] == "function"
    ) {
...
function berocket_wizard_autoselector_end_error() {
    ...
    if (
        typeof functions != "undefined" &&
        functions.any != "" &&
        typeof window[functions.any] == "function"
    ) {
        window[functions.any]();
    }
    if (
        typeof functions != "undefined" &&
        functions.error != "" &&
        typeof window[functions.error] == "function"
    ) {
        window[functions.error]();
    }
    jQuery(".berocket_autoselector").prop("disabled", false);
    BeRocket_set_autoselector_load_position(100);
    jQuery(".berocket_autoselector_load .berocket_autoselector_action").text(
        "Error: " +
            $(
                ".berocket_autoselector_load .berocket_autoselector_action"
            ).text()
    );
    jQuery(".berocket_autoselect_spin").hide();
    jQuery(".berocket_autoselector_load .berocket_line").css(
        "background-color",
        "#bb3333"
    );
    jQuery(".berocket_autoselect_error").show();
    if (jQuery(".berocket_wizard_autoselectors ol li .fa-check").length) {
        jQuery(".berocket_wizard_autoselectors ol li .fa-check")
            .last()
            .removeClass("fa-spinner")
            .addClass("fa-times")
            .removeClass("fa-spin")
